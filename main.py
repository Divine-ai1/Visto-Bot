...
# Messages are counted ONLY in the configured General channel.
#
# Dashboard setting:
#     general_channel
#
# If no General channel has been configured yet, messages
# will NOT be counted.
...
def get_today_message_stats(guild_id, user_id):
    today = datetime.now(
        timezone.utc
    ).strftime("%Y-%m-%d")
...
def build_messages_embed(
    ...
    embed = discord.Embed(
        ...
        description=(
            f"**All Time:** "
            f"{format_stat_number(count)} messages\n"
            f"**Today:** "
            f"{format_stat_number(today_count)} messages\n\n"
...
async def on_message(message):
    ...
    # --------------------------------------------------------
    # GET CONFIGURED GENERAL CHANNEL
    # --------------------------------------------------------

    general_channel = get_message_count_channel(
        guild
    )

    # --------------------------------------------------------
    # COUNT ONLY GENERAL CHANNEL
    # --------------------------------------------------------

    if (
        general_channel is not None
        and message.channel.id == general_channel.id
    ):
...
async def messages_command(
    ...
    channel = get_message_count_channel(
        interaction.guild
    )
    ...
    today_count = get_today_message_stats(
        interaction.guild.id,
        user.id
    )
...
class MessageLeaderboardView(
    ...
    def make_embed(self):
        ...
        if not lines:
            ...
            lines.append(
                "No message statistics recorded yet."
            )
...
async def message_leaderboard_prefix(
    ctx
):
...
async def resetall(
        ...
        return await ctx.send(
            ...
            embed=info_embed(
                ...
                (
                    "Use:\n\n"
                    "`.resetall messages`\n\n"
                    "**This resets message statistics "
    ...
    await ctx.send(
        ...
        embed=warning_embed(
            ...
            (
                "⚠️ This will permanently reset the "
                "**message count for EVERYONE** in this server.\n\n"
...
class ResetMessagesConfirmView(
    discord.ui.View
):
    ...
    async def confirm_reset(
        ...
        if not interaction.user.guild_permissions.administrator:
            ...
            return await interaction.response.send_message(
                embed=error_embed(
                    "No Permission",
                    "Only administrators can reset message statistics."
                ),
                ephemeral=True
            )

        # Make sure this is the same server
        if interaction.guild.id != self.guild_id:

            return await interaction.response.send_message(
                embed=error_embed(
                    "Invalid Server",
                    "This confirmation belongs to another server."
                ),
                ephemeral=True
            )
        ...
        await send_log(
            guild,
            "🗑️ Message Statistics Reset",
            (
                f"**Reset by:** {interaction.user.mention}\n"
                "**Scope:** Everyone\n"
                "**Data:** All message statistics"
            ),
            discord.Color.red()
        )
    ...
    async def cancel_reset(
        ...
        await interaction.response.edit_message(
            ...
            embed=info_embed(
                "Reset Cancelled",
                "No message statistics were changed."
            ),
